...
class MMTRunner(BaseRunner):
    def train_step(self, iter, batch, batch_similarities=None,  multi_loader_similarities=True):
        ...
        lens = [len(inputs_sim) for inputs_sim in inputs_similarities_1]
        lens.insert(0,len(inputs_1))
        for i in range (len(lens)):
            for j in range(i):
                lens[i]+=lens[j]
        if len(inputs_similarities_1)>1:
            inputs_similarities_1 = torch.cat(inputs_similarities_1)
            inputs_similarities_2 = torch.cat(inputs_similarities_2)
        elif len(inputs_similarities_1)==1:
            inputs_similarities_1 = inputs_similarities_1[0]
            inputs_similarities_2 = inputs_similarities_2[0]

        inputs_mo1 = torch.cat((inputs_1,inputs_similarities_1))
        inputs_mo2 = torch.cat((inputs_2,inputs_similarities_2))

        results_1, results_1_mean = self.model[0](inputs_mo1)
        #######Classic Framework -ours-#########
        # results_student_1 = [results_1["feat"][lens[i]:lens[i+1]] for i in range(len(lens)-1)] #for kd optimization
        # results_target_1 , results_source_1 = results_1["feat"][:len_], results_1["feat"][len_:2*len_] #for mmd optimization
        #######-take the mean as a teacher in ours framework-#########
        if self.cfg.KD=="S2P":
            results_student_1 = [results_1["feat"][lens[i]:lens[i+1]] for i in range(len(lens)-1)] #for kd optimization   
        elif self.cfg.KD=="SP":
            results_student_1 = [results_1["activation"][lens[i]:lens[i+1]] for i in range(len(lens)-1)] 
        elif self.cfg.KD=="LwF":
            results_student_1 = [results_1["prob"][lens[i]:lens[i+1]] for i in range(len(lens)-1)]    
        results_target_1 , results_source_1 = results_1["feat"][:len_], results_1_mean["feat"][len_:2*len_] #for mmd optimization


        results_2, results_2_mean = self.model[1](inputs_mo2)
        #######Classic Framework -ours-#########
        # results_student_2 = [results_2["feat"][lens[i]:lens[i+1]] for i in range(len(lens)-1)] #for kd optimization        
        # results_target_2 , results_source_2 = results_2["feat"][:len_], results_2["feat"][len_:2*len_] #for mmd optimization
        #######-take the mean as a teacher in ours framework-#########
        if self.cfg.KD=="S2P":
            results_student_2 = [results_2["feat"][lens[i]:lens[i+1]] for i in range(len(lens)-1)] #for kd optimization   
        elif self.cfg.KD=="SP":
            results_student_2 = [results_2["activation"][lens[i]:lens[i+1]] for i in range(len(lens)-1)] 
        elif self.cfg.KD=="LwF":
            results_student_2 = [results_2["prob"][lens[i]:lens[i+1]] for i in range(len(lens)-1)]          
        results_target_2 , results_source_2 = results_2["feat"][:len_], results_2_mean["feat"][len_:2*len_] #for mmd optimization

        #######-take the mean as a teacher in ours framework-#########
        if task_id>0:
            with torch.no_grad():
                    if self.cfg.KD=="S2P":
                        results_teacher_1 = [results_1_mean["feat"][lens[i]:lens[i+1]] for i in range(len(lens)-1)] 
                        results_teacher_2 = [results_2_mean["feat"][lens[i]:lens[i+1]] for i in range(len(lens)-1)]
                    elif self.cfg.KD=="SP":
                        results_teacher_1 = [results_1_mean["activation"][lens[i]:lens[i+1]] for i in range(len(lens)-1)] 
                        results_teacher_2 = [results_2_mean["activation"][lens[i]:lens[i+1]] for i in range(len(lens)-1)]
                    elif self.cfg.KD=="LwF":
                        results_teacher_1 = [results_1_mean["prob"][lens[i]:lens[i+1]] for i in range(len(lens)-1)] 
                        results_teacher_2 = [results_2_mean["prob"][lens[i]:lens[i+1]] for i in range(len(lens)-1)]
        ...
        #results_2["prob2"] = results_2["prob"]
        ...
        for key in self.criterions.keys():
            if key == "soft_entropy":
                ...
                loss = self.criterions[key](results_source_1,results_target_1) + self.criterions[key](results_source_2,results_target_2)
                # print("loss_mmd {}".format(loss))
            elif key == "kd_loss" or key == "KDLossLwF" or key == "KDLossSP":

                if task_id > 0:
                    results_student_1 = torch.cat(results_student_1)
                    results_teacher_1 = torch.cat(results_teacher_1)
                    results_student_2 = torch.cat(results_student_2)
                    results_teacher_2 = torch.cat(results_teacher_2)
                    loss = self.criterions[key](results_student_1,results_teacher_1)  + self.criterions[key](results_student_2,results_teacher_2) 

                else:
                    loss = loss*0
                # print(f"kd loss at iter {iter} : {loss}")
        ...
        # if (self._epoch % self.cfg.TRAIN.epochs-1 == 0) and (self.epoch > 0) and (iter==self.cfg.TRAIN.iters-1):
        #     print("update model mean net")
        #     with torch.no_grad():
        #         self.model.module._update_mean_net()
        return total_loss
...
def parge_config():
    ...
    if cfg.KD == "LwF":
        cfg.TRAIN.LOSS.losses["KDLossLwF"] = cfg.TRAIN.LOSS.losses.pop("kd_loss")
        if args.KDloss >0:
            cfg.TRAIN.LOSS.losses["KDLossLwF"] = args.KDloss
    elif cfg.KD == "SP":
...
def main():
    ...
    for task in range(n_tasks-1):
        ...
        features = extract_features_for_similarities(runner.model[0], train_loader_similarities, train_sets_similarities)
        similarities,target_similar_source,source_image_ids,source_image_paths = filtre_source_(train_sets_similarities,features,'after task '+str(task+1),writer,rank)
        ...
        model_1 = build_model(cfg, num_classes,[], init=cfg.MODEL.source_pretrained, kd_flag=True)
        model_1.cuda()
        # build model no.2
        model_2 = build_model(cfg, num_classes,[], init=cfg.MODEL.source_pretrained, kd_flag=True)
        model_2.cuda()
        #print(model_1.net.classifier)
        if dist:
            ddp_cfg = {
    ...
    for i, (loader, query, gallery) in enumerate(zip(test_loaders, queries, galleries)):
        ...
        for idx in range(len(runner.model)):
            if idx==1:
                ...
                # test_reid() on self.model[idx] will only evaluate the 'mean_net'
                # for testing 'net', use self.model[idx].module.net
                if cfg.TEST.datasets[i]=="msmt17" and i==0: #msmt17 is too large for testing, uncomment if you want to test on it
                    mAP = 0
                else:
                    cmc, mAP = test_reid(
                        cfg,
