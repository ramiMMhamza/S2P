...
def main():
    ...
    criterions["hybrid_memory"]._update_feature(memory_features,memory_features_target,memory_features_source)
    ...
    #     cmc, mAP = test_reid(
    #         cfg, runner.model, loader, query, gallery, dataset_name=cfg.TEST.datasets[i], visrankactiv= False #cfg.TEST.visrankactiv
    #     )
    #     if rank == 0:
    ...
    for i, (loader, query, gallery) in enumerate(zip(test_loaders, queries, galleries)):
        cmc, mAP = test_reid(
            cfg, model, loader, query, gallery, dataset_name=cfg.TEST.datasets[i], visrankactiv=False
        )
        if rank == 0:
